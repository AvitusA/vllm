...
"""int4(GPTQ) + blockwise-fp8 hybrid dispatch for AutoGPTQConfig.

Checkpoints like Saren/Qwen3.8-Flash-Next-W4A16-AutoRound-hybrid store the
routed experts + lm_head as GPTQ int4/int8 while dense side-layers (GDN
in_proj/out_proj, QSA q/k/v/o, shared experts) are blockwise FP8-e4m3
(`weight` fp8 + `weight_scale_inv`, block 128x128). Stock AutoGPTQConfig
would init those as unquantized bf16 and then fail to load fp8 tensors.

Enabled by VLLM_FP8_HYBRID=1, no-op otherwise. Adapted from
Saren-Arterius/qwen3.8-Flash-DGX-AutoRound src/vllm_fp8_hybrid.py.
"""
import logging
import os
